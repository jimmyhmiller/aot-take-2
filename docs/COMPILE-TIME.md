...
| 10 | Property reads on image objects fold under a closed-world store analysis: a key present in an image object's shape is a load at a fixed offset from its storage word (transitions keep offsets), a key absent from every image shape that no member store or computed store in the program can add folds to `undefined`, and the method a named intrinsic property holds folds to its function when nothing stores that name | The `String.prototype`/`Array.prototype` lookup arms of `JsGetNamed` disappear at sites whose key no image object carries; the budget harness returns to about 4,800 machine nodes | **Landed 2026-09-09** (docs/DECISIONS.md, image object property reads fold under closed-world facts): `aot.node.imagefacts` computes the facts once the world closes; `PropAccess` on a `StaticRef` owner and the [[Prototype]] Load fold in their idealize; a Script's global accesses became these accesses. Budget harness 7,349 → 3,015 machine nodes, 1,611 → 517 blocks, 138 → about 105 ms — below the arrays baseline of 4,811. The history: strings (2026-09-09) had taken 4,811 → 5,429 nodes and 80 → 97 ms; Number and Boolean prototypes 5,429 → 7,349 nodes, 1,197 → 1,611 blocks, 97 → 138 ms |
| 11 | Property descriptors without per-site volume: [[Get]] and [[Set]] as one access each (`PROP-GET`, `PROP-SET`), attribute and result types narrowed by the facts, the store contract, and facts/optimization rounds (docs/DECISIONS.md, property attributes live in the shape tree) | Descriptors and accessors cost nothing at a site whose program defines none; an unfolded site is one runtime call | **Landed 2026-09-09.** The first descriptor version inlined the attribute and chain walk at every site: 3,015 → 15,960 machine nodes, 517 → 3,020 blocks, 105 → 739 ms. Marking the slow builtins noinline: 10,384. The `int[-2..7]` attribute type: 7,792. The store contract (global writes fold again): 7,790. One access per [[Get]]/[[Set]]: 5,502. A store-free slow path plus the second facts round (no object escapes through a dead setter call): **2,914 nodes, 498 blocks, 6 rounds, about 107 ms** — below the pre-descriptor 3,015 |
