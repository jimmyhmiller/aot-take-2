...
| 10 | Property reads on image objects fold under a closed-world store analysis: a key present in an image object's shape is a load at a fixed offset from its storage word (transitions keep offsets), a key absent from every image shape that no member store or computed store in the program can add folds to `undefined`, and the method a named intrinsic property holds folds to its function when nothing stores that name | The `String.prototype`/`Array.prototype` lookup arms of `JsGetNamed` disappear at sites whose key no image object carries; the budget harness returns to about 4,800 machine nodes | **Open.** Strings (2026-09-09) added a prototype lookup arm to every named read on a possibly-string receiver: budget harness 4,811 → 5,429 machine nodes, 80 → 97 ms. Number and Boolean prototypes on primitive receivers (2026-09-09, with Function.prototype call/apply and the Object statics) added the same arm for possibly-number and possibly-boolean receivers: 5,429 → 7,349 machine nodes, 1,197 → 1,611 blocks, 97 → 138 ms — a third of the way from the arrays baseline to the 8,000-node ceiling, so this row is the next lever. `prop-static-storage` recognizes only literals under construction, not `StaticRef` owners |
